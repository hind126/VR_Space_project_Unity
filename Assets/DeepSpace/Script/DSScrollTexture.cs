...
namespace DeepSpace
{
    public class DSScrollTexture : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            {
                Vector2 UVOffs = SharedMaterials[i].GetTextureOffset("_BaseMap");
                UVOffs += (MoveRate[i] * Time.deltaTime);
                UVOffs.x = UVOffs.x % 50;
                UVOffs.y = UVOffs.y % 50;
                SharedMaterials[i].SetTextureOffset("_BaseMap", UVOffs);
            }
        }
        ...
        void OnDestroy()
        {
            for (int i = 0; i < SharedMaterials.Length; i++)
            {
                SharedMaterials[i].SetTextureOffset("_BaseMap", Vector2.zero);
            }
        }
    }
}
